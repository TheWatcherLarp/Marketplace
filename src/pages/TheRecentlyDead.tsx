  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900">
        <p className="text-gray-700 dark:text-gray-300">Loading recently dead characters...</p>
      </div>
    );
  }
            ...
            {deadCharacters.map((character) => (
              <Card key={character.id} className="flex flex-col">
                <CardHeader>
                  ...
                  <CardDescription>
                    Died on: {new Date(character.died_at).toLocaleDateString()}
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex-grow">
                  <p className="text-sm text-gray-700 dark:text-gray-300">Race: {character.race}</p>
                  <p className="text-sm text-gray-700 dark:text-gray-300">Guild: {character.guild}</p>
                  <p className="text-sm text-gray-700 dark:text-gray-300">Branch: {character.branch}</p>
